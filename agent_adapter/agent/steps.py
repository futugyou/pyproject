...
class TaskStepsAgentWithExecution:
    ...
    async def run_agent(self, input_data: dict[str, Any]) -> AsyncGenerator[Any, None]:
        ...
        async for event in self._base_agent.run_agent(input_data):
            ...
            match event:
                case StateSnapshotEvent(snapshot=snapshot):
                    ...
                    print(f"Captured STATE_SNAPSHOT event with state: {final_state}")
                    yield event
                case StateDeltaEvent(delta=delta):
                    ...
                    print(f"Yielding event immediately: {event_type_str}")
                    yield event
        ...
        print(f"Base agent completed. Final state: {final_state}")
        ...
        if final_state and "steps" in final_state:
            ...
            for i in range(len(steps)):
                ...
                print(f"Step {i + 1} marked as completed")
                ...
                print(f"Yielding StateDeltaEvent for step {i + 1}")
                yield delta_event
            ...
            print("Yielding final StateSnapshotEvent with all steps completed")
            yield final_snapshot

            # SECOND LLM call: Stream summary from chat client directly
            print("Making SECOND LLM call to generate summary after step execution")
            ...
            print("Calling chat client for summary")
            ...
            try:
                ...
                print(f"Summary complete: {accumulated_text}")
        ...
        if run_finished_event:
            print("Yielding original RUN_FINISHED event")
