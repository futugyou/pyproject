...
class Ingredient(BaseModel):
    ...
    icon: str = Field(
        ..., description="Emoji icon representing the ingredient (e.g., 🥕)"
    )
...
class Recipe(BaseModel):
    ...
    special_preferences: list[str] = Field(
        ...
        description="Dietary preferences (e.g., Vegetarian, Gluten-free)",
    )
    cooking_time: CookingTime = Field(..., description="The estimated cooking time")
    ingredients: list[Ingredient] = Field(
        ..., description="Complete list of ingredients"
    )
...
def update_recipe(recipe: Recipe) -> str:
    ...
    """
    print("update recipe was called")
...
def get_recipe_agent(chat_client: ChatClientProtocol[Any]) -> AgentFrameworkAgent:
