...
class Recipe(BaseModel):
    ...
    special_preferences: Optional[List[SpecialPreferences]] = Field(
        None, description="A list of special preferences for the recipe"
    )
    cooking_time: Optional[CookingTime] = Field(
        None, description="The cooking time of the recipe"
    )
    ingredients: List[Ingredient] = Field(
        ..., description="Entire list of ingredients for the recipe"
    )
    instructions: List[str] = Field(
        ..., description="Entire list of instructions for the recipe"
    )
    changes: Optional[str] = Field(
        None, description="A description of the changes made to the recipe"
    )
...
def before_model_modifier(
    ...
    if agent_name == "RecipeAgent":
        ...
        if (
            "recipe" in callback_context.state
            and callback_context.state["recipe"] is not None
        ):
            try:
...
def build_recipe_agent(llm: BaseLlm) -> BaseAgent:
    base_agent = LlmAgent(
        name="RecipeAgent",
        model=llm,
        instruction=f"""
